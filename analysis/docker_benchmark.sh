...
if ! docker version &>/dev/null; then
    ...
    # Time launching Docker Desktop separately, properly capturing detailed timing
    /usr/bin/time -v bash -c 'powershell.exe -Command "Start-Process \"Docker Desktop\" -Verb RunAs"' 2>&1 | tee -a "$LOGFILE"
...
{ /usr/bin/time -v docker build -t sort-benchmark -f environments/docker/Dockerfile . > /dev/null 2>&1; } 2>> "$LOGFILE"
