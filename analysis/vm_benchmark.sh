...
{ /usr/bin/time -v sudo apt update && sudo apt install -y python3-pip python3-venv; } 2>> "$LOGFILE"

echo "Creating virtual environment..." | tee -a "$LOGFILE"
{ /usr/bin/time -v python3 -m venv venv; } 2>> "$LOGFILE"

echo "Installing matplotlib in venv..." | tee -a "$LOGFILE"
{ /usr/bin/time -v venv/bin/pip install matplotlib; } 2>> "$LOGFILE"
